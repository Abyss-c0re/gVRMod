...
#   - Skip build when install/native/CubeUI is present and
...
BIN="$ROOT/install/native/CubeUI"
...
if [[ "$force_build" == "1" ]]; then
  need_build=1
  echo "[CubeUI] force build requested (GVMOD_FORCE_BUILD/CUBE_FORCE_BUILD)"
elif [[ "$no_build" == "1" ]]; then
  need_build=0
elif [[ ! -x "$BIN" ]]; then
  need_build=1
  echo "[CubeUI] binary missing — building once"
else
  bin_mtime=$(stat -c '%Y' "$BIN" 2>/dev/null || echo 0)
  src_mtime=$(newest_src_mtime)
  if (( src_mtime > bin_mtime )); then
    need_build=1
    echo "[CubeUI] sources newer than binary — rebuilding (src=$src_mtime bin=$bin_mtime)"
  else
    echo "[CubeUI] binary up to date — skip compile (no source change)"
  fi
fi

if [[ "$need_build" == "1" ]]; then
  if [[ "$no_build" == "1" ]]; then
    echo "[CubeUI] ERROR: GVMOD_NO_BUILD set but binary needs build: $BIN" >&2
    exit 1
  fi
  echo "[CubeUI] building native launcher → $BIN"
  ...
  cmake --build "$BUILD_DIR" -j"$(nproc)" --target CubeUI
fi

if [[ ! -x "$BIN" ]]; then
  echo "[CubeUI] ERROR: $BIN missing after build" >&2
  exit 1
fi

# Ensure project defaults sit next to the binary (copy only if different / missing)
if [[ -f "$SRC_DIR/CubeUI.conf" ]]; then
  mkdir -p "$ROOT/install/native"
  if [[ ! -f "$ROOT/install/native/CubeUI.conf" ]] || \
     ! cmp -s "$SRC_DIR/CubeUI.conf" "$ROOT/install/native/CubeUI.conf" 2>/dev/null; then
    cp "$SRC_DIR/CubeUI.conf" "$ROOT/install/native/CubeUI.conf"
  fi
fi

echo "╔════════════════════════════════════════════════════════════╗"
echo "║  CubeUI — native OpenXR launcher                           ║"
echo "║  TRIGGER click (no hover) · GRIP move · MENU reset pose    ║"
echo "║  passthrough + flexible panel (CubeUI.conf)                ║"
echo "║  Host: echo start|reset|click|addons >/tmp/CubeUI_cmd      ║"
echo "╚════════════════════════════════════════════════════════════╝"
