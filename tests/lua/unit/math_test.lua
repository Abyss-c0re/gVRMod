return function(H, env)
	...
	-- G48 avatar open must not sync-validate / precache the whole list
	H.TEST("util.avatar_load_law.async_g48", function()
		local u = env.vrmod.utils
		H.assert_eq(u.AvatarLoadLaw_MaxValidatePerTick(), 1)
		H.assert_true(not u.AvatarLoadLaw_AllowSyncListFilter())
		H.assert_true(not u.AvatarLoadLaw_AllowSyncPrecacheOnOpen())
		H.assert_true(not u.AvatarLoadLaw_AllowSyncValidateOnOpen())
		H.assert_true(u.AvatarLoadLaw_DeferTwinSpawn())
		H.assert_true(u.AvatarLoadLaw_DeferIkOnOpen())
		H.assert_eq(u.AvatarLoadLaw_PlaceholderModel(), "models/player/kleiner.mdl")
		local ok = u.AvatarLoadLaw_Decide({ phase = "menu_open" })
		H.assert_true(ok.path_ok)
		H.assert_true(ok.open_now)
		H.assert_true(not ok.validate_now)
		H.assert_eq(u.AvatarLoadLaw_StatusLabel(ok), "AVATAR · ASYNC")
		local he = u.AvatarLoadLaw_HmdExpect(ok)
		H.assert_eq(he.verdict, "expect_async")
		H.assert_true(string.find(he.checklist, "G48", 1, true))
		local ready = u.AvatarLoadLaw_Decide({ phase = "twin_open", model_loaded = true })
		H.assert_true(ready.path_ok)
		H.assert_true(not ready.spawn_placeholder)
		H.assert_eq(u.AvatarLoadLaw_StatusLabel(ready), "AVATAR · READY")
		local hold = u.AvatarLoadLaw_Decide({ phase = "twin_open", model_loaded = false })
		H.assert_true(hold.spawn_placeholder)
		H.assert_true(not hold.precache_now)
		H.assert_eq(u.AvatarLoadLaw_StatusLabel(hold), "AVATAR · PLACEHOLDER")
		local freezeList = u.AvatarLoadLaw_Decide({ phase = "menu_open", sync_validate_all = true })
		H.assert_true(not freezeList.path_ok)
		H.assert_eq(freezeList.risk, "sync_list")
		H.assert_true(u.AvatarLoadLaw_IsFreezeRisk(freezeList))
		local freezePc = u.AvatarLoadLaw_Decide({ phase = "twin_open", sync_precache = true })
		H.assert_eq(freezePc.risk, "sync_precache")
		H.assert_true(u.AvatarLoadLaw_IsFreezeRisk(freezePc))
	...
	H.TEST("util.stereo_light_law.per_eye_g49", function()
		local u = env.vrmod.utils
		H.assert_true(u.StereoLightLaw_UpdateProjectedPerEye())
		...
		H.assert_true(left.update_projected)
		H.assert_eq(u.StereoLightLaw_StatusLabel(left), "LIGHT · PER EYE LEFT")
		local right = u.StereoLightLaw_Decide({ eye = "right", vr_active = true })
		H.assert_true(right.refresh)
		H.assert_eq(u.StereoLightLaw_StatusLabel(right), "LIGHT · PER EYE RIGHT")
		local he = u.StereoLightLaw_HmdExpect(right)
		H.assert_eq(he.verdict, "expect_both_eyes")
		H.assert_true(string.find(he.checklist, "G49", 1, true))
		local nest = u.StereoLightLaw_Decide({ eye = nil, vr_active = true })
		H.assert_true(not nest.refresh)
		H.assert_eq(u.StereoLightLaw_StatusLabel(nest), "LIGHT · SKIP NEST")
		local leftOnly = u.StereoLightLaw_Decide({
			eye = "left", vr_active = true, update_left_only = true,
		})
		H.assert_true(not leftOnly.path_ok)
		H.assert_eq(leftOnly.risk, "right_eye_only")
		H.assert_true(u.StereoLightLaw_IsRightEyeOnlyRisk(leftOnly))
		H.assert_eq(u.StereoLightLaw_StatusLabel(leftOnly), "LIGHT · RIGHT EYE ONLY")
		local pre = u.StereoLightLaw_Decide({
			eye = "left", vr_active = true, update_pre_stereo_only = true,
		})
	...
	-- G51 Submit-first frame budget (no third RenderView / collect on late frames)
	H.TEST("util.frame_budget_law.submit_first_g51", function()
		local u = env.vrmod.utils
		H.assert_eq(u.FrameBudget_DesktopSkipMs(), 14)
		H.assert_true(not u.FrameBudget_InputAfterSubmit())
		H.assert_true(not u.FrameBudget_FollowCamAfterSubmit())
		H.assert_true(not u.FrameBudget_AllowNestedMonitorsInFollowCam())
		local ok = u.FrameBudget_Decide({ frame_ms = 8, desktop_view = 3 })
		H.assert_true(ok.path_ok)
		H.assert_true(not ok.skip_desktop)
		H.assert_true(not ok.skip_collect)
		H.assert_eq(u.FrameBudget_StatusLabel(ok), "BUDGET · ON TIME")
		local he = u.FrameBudget_HmdExpect(ok)
		H.assert_eq(he.verdict, "expect_on_time")
		H.assert_true(string.find(he.checklist, "G51", 1, true))
		local late = u.FrameBudget_Decide({ frame_ms = 18, desktop_view = 3, eye_crop = true })
		H.assert_true(late.late)
		H.assert_true(not late.skip_desktop)
		H.assert_true(not late.skip_collect)
		H.assert_true(u.FrameBudget_IsLateRisk(late))
		local lateFollow = u.FrameBudget_Decide({ frame_ms = 18, desktop_view = 4, follow = true })
		H.assert_true(lateFollow.skip_desktop)
		H.assert_true(not lateFollow.skip_collect)
		H.assert_eq(u.FrameBudget_StatusLabel(late), "BUDGET · LATE SKIP DESKTOP")
		H.assert_eq(u.FrameBudget_HmdExpect(late).verdict, "expect_skip_desktop")
	end)
end
